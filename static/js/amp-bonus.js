{
    ...
    if(btn) {
         ...
         const dataSetReadMore = btn.attributes.filter((item) => item.name === 'data-translate-read-more')
         const readMore = dataSetReadMore.length ? dataSetReadMore[0].value : ""
 
         const dataSetGetBonus = btn.attributes.filter((item) => item.name === 'data-translate-get-bonus')
         const getBonus = dataSetGetBonus.length ? dataSetGetBonus[0].value : ""
         
         const dataSetBonusInvalid = btn.attributes.filter((item) => item.name === 'data-translate-bonus-invalid')
         const bonusInvalid = dataSetBonusInvalid.length ? dataSetBonusInvalid[0].value : ""
         ...
         function renderTemplate(posts, counter) {
                 ...
                 for(let i = counter * postsOnQuery; i<length; i++) {
                    ...
                    if(posts[i].close === 0) {
                        html += `<a href="${getRef(posts[i])}" class="bonuses-item__btn btn-primary">
                            ${getBonus}
                        </a>`
                    } else {
