...
<div class="casino-card">
    <div class="container casino-card__container">
        <div class="casino-card__logo">
            <amp-img :src="value.thumbnail" :alt="value.title" width="250" height="156" />
        </div>
            ...
            <div class="detail-table detail-table--2-col">
                <div class="detail-row">
                    <div class="detail-row-item" v-if="value.country.length !== 0">
                        <div class="detail-row__heading">{{translates.COUNTRY_REPRESENTATIVE[config.LANG]}}</div>

                        <div class="detail-row__content">
                            <NuxtLink class="detail-item" 
                               :to="`${config.AMP_PREFIX}${value.country[0].permalink}`">{{value.country[0].title}}</NuxtLink>
                        </div>
                    </div>

                    <div class="detail-row-item" v-if="value.year !== ''">
                        <div class="detail-row__heading">{{translates.YEAR_FOUNDATION[config.LANG]}}</div>
                    ...
                    <div class="detail-row-item" v-if="value.number_games !== ''">
                        <div class="detail-row__heading">{{translates.NUMBER_GAMES[config.LANG]}}</div>
