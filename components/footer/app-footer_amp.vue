...
<footer class="footer">
    <div class="container">
        ...
        <NuxtLink v-for="(item, index) in footer_menu" :key="index" :to="`${config.AMP_PREFIX}${item.value_1}`"
                class="footer-nav-link">{{item.value_2}}</NuxtLink>
        </div>
    </div>
    <div class="partners-logos">
        <div class="container" v-if="partners_menu.length !== 0">
            <ul class="partners-logos__list">
                <li class="partners-logos__item" v-for="(item, index) in partners_menu" :key="index">
                    <a :href="item.value_1">
